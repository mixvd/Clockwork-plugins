...
local COMMAND = Clockwork.command:New("Hut");
COMMAND.tip = "Speak in Huttese.";
...
function COMMAND:OnRun(player, arguments)
	if (Clockwork.player:HasFlags(player, "H")) then
		...
		for k,v in pairs(_player.GetAll()) do
			...
			if (math.floor(distance) <= Clockwork.config:Get("talk_radius"):Get()) then
				if (Clockwork.player:HasFlags(v, "H")) then			
	Clockwork.chatBox:SendColored(v, Color(134, 214, 122, 255), "[Huttese] "..player:Name().." says: "..speech)
			else
		...
		Clockwork.player:Notify(player, "You do not speak Huttese!")
	end;
end;
