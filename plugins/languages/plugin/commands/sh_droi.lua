...
local COMMAND = Clockwork.command:New("Droi");
COMMAND.tip = "Speak in Droid.";
	...
	if (Clockwork.player:HasFlags(player, "d")) then
		...
		for k,v in pairs(_player.GetAll()) do
			...
			if (math.floor(distance) <= Clockwork.config:Get("talk_radius"):Get()) then
				if (Clockwork.player:HasFlags(v, "d")) then			
	Clockwork.chatBox:SendColored(v, Color(122, 174, 214, 255), "[Droidspeak] "..player:Name().." says: "..speech)
			else
		...
		Clockwork.player:Notify(player, "You do not speak Droidspeak!")
	end;
end;
